        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-4 gap-6 items-stretch">
          <div 
            *ngFor="let pkg of packages"
            [class]="pkg.isPopular 
              ? 'relative bg-slate-800/90 border-2 border-brand-ice rounded-3xl p-6 lg:p-8 flex flex-col justify-between shadow-[0_0_30px_rgba(56,189,248,0.15)]' 
              : 'relative bg-slate-900 border border-slate-800 rounded-3xl p-6 lg:p-8 flex flex-col justify-between hover:border-slate-700 transition'"
          >
            <span *ngIf="pkg.isPopular" class="absolute -top-3.5 left-1/2 -translate-x-1/2 bg-brand-ice text-slate-950 font-extrabold text-xs px-4 py-1 rounded-full uppercase tracking-wider whitespace-nowrap">
              Most Popular
            </span>
            ...
            <div>
              ...
              <div class="flex items-baseline gap-1 mb-6">
                ...
                <span class="text-slate-400 text-xs lg:text-sm">{{ pkg.duration }}</span>
              </div>
            ...
            <a 
              ...
              target="_blank"
              [class]="pkg.isPopular 
                ? 'w-full py-3.5 rounded-xl font-bold bg-brand-ice text-slate-950 text-center hover:bg-sky-300 transition shadow mt-auto' 
                : 'w-full py-3.5 rounded-xl font-bold bg-slate-800 text-white text-center hover:bg-slate-700 transition mt-auto'"
            >
              Choose Plan
            </a>
          </div>
